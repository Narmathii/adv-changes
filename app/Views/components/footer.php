...
<script>
    ...
    // Get text from the specific input that triggered the event
    var searchText = $(inputElement).val().trim();
    
    // Find the closest parent form and then find the resultBox inside it
    var suggestionsBox = $(inputElement).closest('form').find('.resultBox');
    ...
    if (searchText.length > 0) {
        $.ajax({
            ...
            success: function (response) {
                // Pass both the response AND the specific box to display
                displaySuggestions(response, suggestionsBox, $(inputElement));
            },
            error: function () {
                suggestionsBox.html('<div class="no-suggestions">Error fetching suggestions</div>');
...
function displaySuggestions(suggestions, suggestionsBox, inputField) {
    ...
    if (suggestions.length > 0) {
        suggestions.forEach(function (suggestion) {
            const redirectUrl = suggestion.redirect_url.toLowerCase().replace(/\s+/g, '-');
            const encodedId = btoa(suggestion.prod_id);

            let url = "#";
            switch (suggestion.tbl_name) {
                case "tbl_products": url = `${base_Url}detail/${redirectUrl}/${encodedId}`; break;
                case "tbl_accessories_list": url = `${base_Url}accessories-detail/${redirectUrl}/${encodedId}`; break;
                case "tbl_rproduct_list": url = `${base_Url}riding-details/${redirectUrl}/${encodedId}`; break;
                case "tbl_helmet_products": url = `${base_Url}helmet-details/${redirectUrl}/${encodedId}`; break;
                case "tbl_luggagee_products": url = `${base_Url}tour-detail/${redirectUrl}/${encodedId}`; break;
                case "tbl_camping_products": url = `${base_Url}camp-details/${redirectUrl}/${encodedId}`; break;
            }
            ...
            // Use the specific inputField and suggestionsBox passed to the function
            searchResHtml.on('click', function () {
                inputField.val(suggestion.product_name);
                suggestionsBox.empty().addClass('d-none');
            });

            suggestionsBox.removeClass("d-none").append(searchResHtml);
        });
    } else {
        suggestionsBox.removeClass('d-none');
