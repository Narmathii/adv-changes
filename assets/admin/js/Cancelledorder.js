$(document).ready(function () {
  ...
  $(document).on("click", ".orderDetails", function () {
    ...
    $.ajax({
      type: "POST",
      data: { order_id: orderid },
      url: base_Url + "get-cancelorder-details",
      success: function (data) {
        ...
        if (!viewOrder || viewOrder.length === 0) {
          // If empty, show a message or just exit
          $("#order-details tbody").html(
            `<tr><td colspan="9" class="text-center">No order details found.</td></tr>`
          );
          return;
        }

        // Existing logic for non-empty response
        let sizee = viewOrder.length;
        CancelStatusID = viewOrder[0]["order_id"];

        let DisplayBtn = `<a type="button" id="refund-btn" class="btn btn-warning my-1 me-2">Click to Refund</a>`;

        if (viewOrder[0]["payment_status"] === "COMPLETED") {
          $(".refund_menu").removeClass("d-none");
          $("#refund-button").removeClass("d-none").html(DisplayBtn);
        }

        let RefundStatus = `<a type="button" id="refund-status" class="btn btn-info my-1 me-2">Check Refund Status</a>`;
        if (viewOrder[0]["delivery_status"] == "Refund Processing") {
          $(".refund_menu").removeClass("d-none");
          $("#refund-status").removeClass("d-none").html(RefundStatus);
        }
        ...
        var status = "";
        switch (viewOrder[0]["payment_status"]) {
          case "FAILED":
          case "CANCELLED":
        ...
        // Cancel reason
        $(".cancel-reason").html(
          `<h6>Cancel Reason</h6><p>${viewOrder[0]["cancel_reason"]}</p>`
        );
        ...
        for (let i = 0; i < sizee; i++) {
          ...
          let dispOffer =
            OfferType === ""
              ...
              ? "Flat Discount"
              : "none";
          let dispDropShipping =
            viewOrder[0]["drop_shipping"] == 1 ? "DropShipping" : "Retail";

          tblData += `
      <tr>
        <td><div class="mb-1 fs-14 fw-semibold"><a href="javascript:void(0);">${
          i + 1
        }.</a></div></td>
        <td>
          <div class="d-flex align-items-center">
            <div class="me-3">
              <span class="avatar avatar-xxl bg-light">
                <img src="${base_Url}${viewOrder[i]["config_image1"]}" alt="">
              </span>
            </div>
          </div>
        </td>
        <td>
          <div class="mb-1 fs-12 fw-semibold order-prodname"><a href="javascript:void(0);">${
            viewOrder[i]["product_name"]
          }</a></div>
          <div class="fs-12 fw-semibold size ${className}"><p>Size : ${size}</p></div>
          <div class="fs-1 fw-semibold color ${classNamee}"><p>Color : ${color}</p></div>
        </td>
        <td><span class="fs-15 fw-semibold">${
          viewOrder[i]["actual_price"]
        }</span></td>
        <td><span class="badge bg-danger-gradient">${dispOffer}</span></td>
      ...
      <tr>
        <td colspan="3"></td>
        <td colspan="3"><div class="fw-semibold" style="text-align: right;">Delivery Charge(${viewOrder[0]["courier_type"]}) :</div></td>
        <td colspan="3" style="text-align: right;"><span class="fs-16 fw-semibold"> ₹${viewOrder[0]["courier_charge"]}</span></td>
      </tr>
      <tr>
        <td colspan="3"></td>
        <td colspan="3"><div class="fw-semibold" style="text-align: right;">Total Price :</div></td>
        <td colspan="3" style="text-align: right;"><span class="fs-16 fw-semibold"> ₹${viewOrder[0]["sub_total"]}</span></td>
      </tr>`;
