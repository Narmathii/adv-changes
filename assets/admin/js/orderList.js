$(document).ready(function () {
  ...
  $("#filter-btn").click(function () {
    ...
    $.ajax({
      type: "POST",
      url: base_Url + "filter-orders",
      data: { filter_date: filterDate },
      dataType: "json",

      success: function (data) {
        dispOrderDetails(data);
      },
  ...
  $("#payment-sts-btn").click(function () {
    let orderID = $("#payment-sts-btn").attr("order-id");

    $.ajax({
      type: "POST",
      url: base_Url + "check-payment-status",
      ...
      success: function (data) {
        if (data.code == 200) {
          ...
          status += `Payment Status - <span class="badge bg-success">${viewOrder[0]["payment_status"]}</span>`;
          $("#payment-sts").html("" + status);
          $("#payment-sts-btn").css("display", "none");
